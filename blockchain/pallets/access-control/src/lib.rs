...
pub mod pallet {
    ...
    // `Role` appears in the genesis config, which is patched in as JSON.
    // Derived unconditionally, and via frame_support's re-export rather than a
    // direct serde dependency: `#[pallet::genesis_config]` emits
    // `frame_support::Serialize`/`Deserialize` on the generated struct with no
    // `std` gate, so gating these on `feature = "std"` breaks the no_std wasm
    // build of the runtime while leaving the native build green.
    ...
    /// Upper bound on roles assignable at genesis (NASA Power of 10 rule 2:
    /// the genesis loop below must have a fixed upper bound).
    pub const MAX_GENESIS_ROLES: usize = 64;

    /// Seeds the initial role table.
    ///
    /// Without this, `UserRoles` is unreachable. `assign_role` is the only
    /// writer, it requires `is_admin(caller)`, and it refuses to assign
    /// `Role::Admin` (`CannotAssignAdmin`) — so on a chain with an empty role
    /// table no account can ever become Admin, no roles can ever be assigned,
    /// and every call gated on `is_healthcare_provider` /
    /// `can_edit_medical_records` fails permanently. That includes the two
    /// calls the MediChain API submits:
    /// `MedicalRecords::upsert_emergency_capsule_commitment` and
    /// `AccessControl::log_delegated_access`.
    ///
    /// The pallet's own `assign_role` docs already said "Admin role can only be
    /// set at genesis" — this is the genesis path that sentence assumed.
    #[pallet::genesis_config]
    #[derive(frame_support::DefaultNoBound)]
    pub struct GenesisConfig<T: Config> {
        /// Accounts granted a role at genesis, e.g. the founding Admin and the
        /// backend's operator key. Development chain specs populate this; a
        /// production chain spec must set it deliberately.
        pub initial_roles: Vec<(T::AccountId, Role)>,
    }
    ...
    impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
        fn build(&self) {
            assert!(
                self.initial_roles.len() <= MAX_GENESIS_ROLES,
                "genesis initial_roles exceeds MAX_GENESIS_ROLES",
            );
            for (account, role) in self.initial_roles.iter().take(MAX_GENESIS_ROLES) {
    ...
    pub enum Event<T: Config> {
        ...
        /// An application access submitted by a trusted operator on behalf of
        /// an off-chain accessor. Only keyed commitments leave the API.
        DelegatedAccessLogged {
            patient: T::AccountId,
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// Record an application-level access without pretending that the
        /// operator signer was the clinician or device that performed it.
        #[pallet::call_index(6)]
        #[pallet::weight(T::WeightInfo::grant_emergency_access())]
        pub fn log_delegated_access(
            origin: OriginFor<T>,
            patient: T::AccountId,
            accessor_hash: [u8; 32],
            reason_hash: [u8; 32],
            ...
            Self::deposit_event(Event::DelegatedAccessLogged {
                patient,
                submitted_by,
                accessor_hash,
                reason_hash,
